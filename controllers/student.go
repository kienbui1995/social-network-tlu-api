...
func (controller StudentController) GetAll(c *gin.Context) {
	...
	helpers.ResponseEntityListJSON(c, 1, "Get student list successful", students, params, len(students))
}
...
func (controller StudentController) UpdateFromTLU(c *gin.Context) {
	fromClass, _ := strconv.ParseInt(c.Query("from"), 10, 64)
	toClass, _ := strconv.ParseInt(c.Query("to"), 10, 64)
	...
	// 	return
	// }
	for index := fromClass; index <= toClass; index++ {
...
func (controller StudentController) GetStudentsEnrolledClass(c *gin.Context) {
	...
	params.Sort = c.DefaultQuery("sort", configs.SSort)
	params.Sort, _ = helpers.ConvertSort(params.Sort)
	students, errGetAll := controller.Service.GetStudentsEnrolledClass(params, code)
	if errGetAll != nil {
		helpers.ResponseServerErrorJSON(c)
		fmt.Printf("GetAll service: %s\n", errGetAll.Error())
		return
