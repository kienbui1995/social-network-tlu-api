...
func (controller SemesterController) GetAll(c *gin.Context) {
	...
	semesters, errGetAll := controller.Service.GetAll(params)
	if errGetAll != nil {
...
func (controller SemesterController) UpdateFromTLU(c *gin.Context) {
	fromYear, _ := strconv.ParseInt(c.Query("from"), 10, 64)
	toYear, _ := strconv.ParseInt(c.Query("to"), 10, 64)
	...
	// 	return
	// }
	for index := fromYear; index <= toYear; index++ {
	...
	helpers.ResponseSuccessJSON(c, 1, "Update user successful", nil)
}
...
func (controller SemesterController) GetSemesterOfStudent(c *gin.Context) {
	...
	params.Sort = c.DefaultQuery("sort", configs.SSort)
	params.Sort, _ = helpers.ConvertSort(params.Sort)
	semesters, errGetAll := controller.Service.GetAllOfStudent(params, studentCode)
	if errGetAll != nil {
		helpers.ResponseServerErrorJSON(c)
		fmt.Printf("GetAll service: %s\n", errGetAll.Error())
		return
